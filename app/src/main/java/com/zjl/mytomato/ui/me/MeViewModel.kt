...
class MeViewModel : BaseViewModel() {
    ...
    fun updateToNetWork(localTodoEntities: MutableSet<TodoEntity>, localFinishTodoEntities: MutableSet<FinishTodoEntity>,localTimedTaskEntities:MutableSet<TimedTaskEntity>, username: String) {
    ...
    private fun innerUpdateTimedTaskTodo(localTimedTaskEntities: MutableSet<TimedTaskEntity>, username: String) {
        BmobQuery<NTimedTaskEntity>().addWhereEqualTo("username", username).findObjects(object : FindListener<NTimedTaskEntity>() {
            override fun done(networkdatas: MutableList<NTimedTaskEntity>?, p1: BmobException?) {
    ...
    private fun innerUpdateFinishTodo(localFinishTodoEntities: MutableSet<FinishTodoEntity>, username: String) {
        BmobQuery<NFinishTodoEntity>().addWhereEqualTo("username", username).findObjects(object : FindListener<NFinishTodoEntity>() {
            override fun done(networkdatas: MutableList<NFinishTodoEntity>?, p1: BmobException?) {
                if (networkdatas != null) {
                    var size = 0
                    for (item in networkdatas) {
