                    ...
                    when (event.action) {
                        ...
                        MotionEvent.ACTION_UP -> {
                            if (v.translationX <= (v.right - v.left) / 2) {
                                val animator = ObjectAnimator.ofFloat(v, "translationX", v.translationX, 0f)
                                animator.duration = 500
                                animator.interpolator = BounceInterpolator()
                                animator.start()
                            } else {
                                CommonDialog(context, content = "确定要删除此任务吗", listener = object : CommonDialog.DialogClickListener {
